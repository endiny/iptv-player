...
export const ChannelDetails: React.FC = () => {
  ...
  const currentProgramme = useMemo(
    () => getCurrentProgramme(channel?.tvg?.id ?? ''),
    [channel, getCurrentProgramme],
  );
  ...
  return (
    <div className="mx-4 mb-2 flex items-center gap-3 rounded-lg bg-black/50 p-3 backdrop-blur-sm md:mx-6 md:max-w-3xl">
      {logoSrc ? (
        ...
        <div className="flex h-14 w-14 items-center justify-center rounded bg-white/10 text-xl md:h-16 md:w-16">
          📺
        </div>
      )}
